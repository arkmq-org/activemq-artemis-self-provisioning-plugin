...
> = (prevFormState, action) => {
  ...
  switch (action.operation) {
    ...
    case ArtemisReducerOperations712.setAcceptorSecret: {
      ...
      if (!action.payload.secret) return formState;
...
const updateConfigBindToAllInterfaces = (
  ...
  if (configType === ConfigType.acceptors && spec.acceptors) {
    for (let i = 0; i < spec.acceptors.length; i++) {
      if (spec.acceptors[i].name === configName) {
        spec.acceptors[i].bindToAllInterfaces = bindToAllInterfaces;
      }
    }
  }
  if (configType === ConfigType.connectors && spec.connectors) {
    for (let i = 0; i < spec.connectors.length; i++) {
      if (spec.connectors[i].name === configName) {
        spec.connectors[i].bindToAllInterfaces = bindToAllInterfaces;
      }
    }
  }
};
...
const updateConfigSSLEnabled = (
  ...
  if (configType === ConfigType.connectors) {
    const connectors = brokerModel.spec?.connectors ?? [];
    if (connectors.length > 0) {
      for (let i = 0; i < connectors.length; i++) {
        if (connectors[i].name === configName) {
          connectors[i].sslEnabled = isSSLEnabled;
          if (!isSSLEnabled) {
...
const clearAcceptorCertManagerConfig = (cr: BrokerCR, name: string) => {
  ...
  const sslSecret = acceptor.sslSecret;
  if (sslSecret && sslSecret.endsWith('-ptls')) {
  ...
  if (!spec.resourceTemplates) return;

  if (spec.resourceTemplates.length === 0) {
    delete spec.resourceTemplates;
  }
};
